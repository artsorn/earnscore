...
REVIEWER_DIFF_HASH_FILE="${REVIEWER_DIFF_HASH_FILE:-$RUNTIME_DIR/reviewer-diff.sha256}"
REVIEWER_DIFF_META_FILE="${REVIEWER_DIFF_META_FILE:-$RUNTIME_DIR/reviewer-diff.meta.json}"
...
prepare_reviewer_diff() {
  local task="${1:-}" round="${2:-0}" f candidate content_hash previous_hash changed_state
  mkdir -p "$RUNTIME_DIR"
  ...
  candidate="$(mktemp "$RUNTIME_DIR/reviewer-diff.XXXXXX.patch")"
  {
    echo "# Reviewer Diff"
  ...
  content_hash="$(sha256sum "$candidate" | awk '{print $1}')"
  previous_hash=""
  if [[ -f "$REVIEWER_DIFF_HASH_FILE" ]]; then
    previous_hash="$(tr -d '[:space:]' < "$REVIEWER_DIFF_HASH_FILE" 2>/dev/null || true)"
  fi
  if [[ -f "$REVIEWER_DIFF_FILE" && "$content_hash" == "$previous_hash" ]] && cmp -s "$candidate" "$REVIEWER_DIFF_FILE"; then
    rm -f "$candidate"
    changed_state="unchanged"
    scope_event "reviewer diff reused hash=$content_hash task=$task round=$round"
  else
    mv "$candidate" "$REVIEWER_DIFF_FILE"
    printf '%s\n' "$content_hash" > "$REVIEWER_DIFF_HASH_FILE"
    changed_state="changed"
    scope_event "reviewer diff updated hash=$content_hash task=$task round=$round"
  fi
  python3 - "$REVIEWER_DIFF_META_FILE" "$content_hash" "$changed_state" "$task" "$round" "$REVIEWER_DIFF_FILE" <<'PY'
import datetime, json, os, sys
out, digest, state, task, round_no, diff_file = sys.argv[1:]
data = {
    "hash": digest,
    "state": state,
    "task": task,
    "round": int(round_no or 0),
    "bytes": os.path.getsize(diff_file),
    "lines": sum(1 for _ in open(diff_file, encoding="utf-8", errors="replace")),
